...
if os.path.exists(f"../results/lgbm_bagging_fraction.csv"):
    logger = open(f"../results/lgbm_bagging_fraction.csv", "a")
else:
    logger = open(f"../results/lgbm_bagging_fraction.csv", "w")
...
def objective(trial):
    sampled_params = {
        "bagging_fraction": trial.suggest_discrete_uniform("bagging_fraction", 0.4, 1.0, 0.1),
    }
